...
uniform float time;
const float SPHERE_RADIUS = 150.0;

const int NUM_HOLES = 5;
uniform vec3 holes[NUM_HOLES];

void main(void) {
    if(vTextureCoord.y < .5) {
		if(vTextureCoord.x < .5) {	//	positions
			...
			pos = normalize(pos) * SPHERE_RADIUS;
			gl_FragColor = vec4(pos, 1.0);
		} else {
			vec2 uvPos = vTextureCoord - vec2(.5, .0);
			vec2 uvOffset = vTextureCoord + vec2(-.5, .5);
			...
			float posOffset = .007 * mix(offsets.r, 1.0, .75);
			float noiseStrength = .2 * mix(offsets.g, 1.0, .5);

			float ax = snoise(pos.x*posOffset + time*.01, pos.y*posOffset + time*.01, pos.z*posOffset + time*.01);
			float ay = snoise(pos.y*posOffset + time*.01, pos.z*posOffset + time*.01, pos.x*posOffset + time*.01);
			float az = snoise(pos.z*posOffset + time*.01, pos.x*posOffset + time*.01, pos.y*posOffset + time*.01);

			vel += vec3(ax, ay, az) * noiseStrength;

			// vec3 AVOID = vec3(0.0, 0.0, -SPHERE_RADIUS);
			for(int i=0; i<NUM_HOLES; i++) {
				vec3 AVOID = normalize(holes[i]) * SPHERE_RADIUS;
				const float minR = 75.0;
				float dist = distance(AVOID, pos);
				if(dist < minR) {
					vec3 dir = normalize(pos-AVOID);
					float f = 1.0 - dist / minR;
					vel += f * dir ;
				}
			}
